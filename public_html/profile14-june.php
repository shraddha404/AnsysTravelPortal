...
<!-- form starts here -->
  <form id="profile" method="post" action="" enctype="multipart/form-data">
    ...
    <!-- Personal details-->
    <div class="row">            <?php echo "<h3 align='right'><p style='color:green;'><a href='emp-board.php'>Dashboard</a></p></h3>"; ?>
    ...
    <fieldset name "personal-info">
		...
		<p><label for "fname">First Name <font color="red">*</font></span></label>
          	...
          	<p><label for "bunit">Organisation Unit <font color="red">*</font></label>
           	...
           	<select name="ou" id="biz_unit" ><option value="">Organisation Unit</option><?php $bizunit=$u->getous($user_profile['ou']); 
		    foreach($ou as $ous) { ?>
		      <option value="<?php echo $ous['id']; ?>" <?php if($ous['ou_short_name']==$bizunit['ou_short_name']){echo 'selected=selected';} ?>><?php echo $ous['ou_short_name']; ?></option>
		  <?php
		    } ?> </select>
         ...
         <p><label for "off-loc">Main Office Location </label>
         <input type="text" id="off-loc" name="location" placeholder="Main Office Location" tabindex="5" value="<?php echo $user_profile['location']; ?>" autofocus  /></p>
...
<input type="text" id="manager" name="manager_email" placeholder="manager email" tabindex="12" value="<?php echo  $manager_email['email']; ?>" autofocus />
</p>
    ...
    <div class="col-3-grid">
        ...
        <input type="file" id="pp-copy" name="passport_copy" placeholder="Upload passport copy" autofocus /></p>
    </div>
    </fieldset>
    <!--/div-->
...
<td><?php echo $visaDetail['visa_no'];?></td>
<td><?php echo $visaDetail['visa-country'];?></td>
<td><?php echo $visaDetail['visa-exp-date'];?></td>
<td><a href="uploads/visa-copy/<?php echo $visaDetail['visa_copy'];?>" target="_blank">View visa_copy</a></td>
<td><input type="hidden" name="visa_id" value="<?php echo $visaDetail['id']; ?>">
<!--input type="submit" class="trash" name="delete_visacopy" value="Delete"><?php echo $visaDetail['id']; ?>-->
<!--a href="#" id="<?php echo $visaDetail['id']; ?>" class="trash">Delete<?php echo $visaDetail['id']; ?></a-->

<input type='button' class="delete_button" id="delete_button<?php echo $visaDetail['id']; ?>" value="delete" onclick="delete_row('<?php echo $visaDetail['id'];?>');">
  </td>
</tr>
<?php } ## foreach ends ?>
</table>
...
<td><?php echo $getffDetail['ffp-id'];?></td>
<td><?php $airline = $u->getairlines($getffDetail['air-co']); echo $airline['name'];?></td>
<td><?php echo $getffDetail['ffp-name'];?></td>
<td><input type="hidden" name="ffp_id" value="<?php echo $getffDetail['id'];?>"> <!--input type="submit" name="delete_ffp" value="Delete"-->
...
<!--select name="air-co[]"><option value="Air India">Air India</option> </select---> </p></div>
    ...
    <div class="row cent"><input type="SUBMIT" value="SUBMIT" /><?php  $type=$u->getUserType();if($type=='Employee'){?> <input type="SUBMIT"value="CANCEL"onclick="javascript:window.location='emp-board.php';"><?php }?>
<?php $type=$u->getUserType();if($type=='Manager'){?><input type="SUBMIT" value="CANCEL" onclick="javascript:window.location='manager-board.php';"><?php }?>
<?php $type=$u->getUserType();if($type=='Travel Desk'){?><input type="SUBMIT" value="CANCEL"onclick="javascript:window.location='travel-desk-board.php';"><?php }?>
<?php $type=$u->getUserType();if($type=='Admin'){?><input type="SUBMIT" value="CANCEL" onclick="javascript:window.location='admin-board.php';"><?php }?></div>
 </form>
