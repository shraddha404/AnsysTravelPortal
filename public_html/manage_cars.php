...
if(!empty($_GET['id'])){
	$selection = $u->getCarDetails($_GET['id']);
}
$cars=$u->getCarscom();
$cities = $u->cities();
?>
...
<?php include 'include/admin_menu.php'; ?>
  <div role="main">
  <form id="profile" method="post" action="/manage_cars.php">
  	<div class="row cent">
	    <div class="in-bloc cent">
		<h1 class="in-bloc">Manage Cars</h1></div>
        </div>
    <div class="row">
<h3 align='right'><p style='color:green;'>
    ...
    <span class="f-leg"><img src="img/user32.png" />Add Car</span>
    <fieldset name "personal-info">
         <div class="col-3-grid"><p><label for "fname">Car</label>
           <input type="text" id="name" name="name" placeholder="Name of the Car Company" tabindex="1" value="<?php echo $selection['name']; ?>" autofocus /></p>
...
<table class="resp">
	...
	<tr>
		<td data-th="Car"><?php echo $a['name']; ?></td>
		
		<td data-th="Action" align="center">
		<form method="post" action="/manage_cars.php">
		<input type="hidden" name="id" value="<?php echo $a['id']; ?>" />
		<input type="submit" name="action" value="DELETE" />
		<a href="/manage_cars.php?action=update&id=<?php echo $a['id']; ?>"><input type="button" name="action" value="UPDATE" /></a>
		</form>
		</td>
	</tr>
	<?php } ?>
